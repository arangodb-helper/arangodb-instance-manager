...
export default class VersionResponse {
  ...
  constructor(response: any) {
    ...
    if (Object.keys(response).length !== 3) {
      throw new TypeError(
        `Unexpected properties in response, response is ${JSON.stringify(
          response
        )}`
      );
    }
    ...
    if (response.server !== "arango") {
      throw new TypeError(
        `Unexpected property .server=${response.server}; expected "arango".`
      );
    }
    if (response.license !== "community" && response.license !== "enterprise") {
      throw new TypeError(
        `Unexpected property .license=${response.license}; expected "community" or "enterprise.`
      );
    }
    const versionRe = /^(\d+)\.(\d+)\.(.*)$/;
    let versionReResult;
    if (
      ...
      !(versionReResult = versionRe.exec(response.version))
    ) {
      // Be a bit lenient here, allow e.g. 3.4.devel. But we do need major and minor version.
      throw new TypeError(
        `Unexpected property .version=${response.version}; expected a version string.`
      );
    }
